...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  N, T, D = x.shape # 2,3,4
  H = Wh.shape[0]
  # As "next timesteps" only depends on the previous timesteps, so compute all the first steps together.
  x = x.transpose(1,0,2) #T, N, D (3,2,4)
  prev_h = h0
  h = np.zeros((T,N,H)) #(3,2,5)
  h[0] = h0  #(2,5)
  cache = []
  for t in xrange(T):
