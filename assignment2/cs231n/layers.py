...
def affine_backward(dout, cache):
  ...
  dx = np.reshape(dx, np.shape(x)) # (N, d1, ..., d_k)
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  # conv layer size: F, C, HH, WW
  # where HH and WW are filter's height and width
  conv_h = 1 + (H + 2*pad - HH) / stride
  conv_w = 1 + (W + 2*pad - WW) / stride
  x_padded = np.pad(x, [(0,0),(0,0),(pad,pad),(pad,pad)], 'constant')

  # take W=H=7, HH=WW=3, S=2
  # conv_h = conv_w = 3
  out = np.zeros((N, F, conv_h, conv_w))
  for n in xrange(N):
      for f in xrange(F):
          for hh in xrange(conv_h):  #0,1,2
              for ww in xrange(conv_w):
...
def conv_backward_naive(dout, cache):
  ...
  for n in xrange(N):
      for f in xrange(F):
          for hh in xrange(conv_h):
              for ww in xrange(conv_w):
                  field = x_padded[n,:,(hh*stride):(hh*stride+HH),(ww*stride):(ww*stride+WW)]
                  dx_padded[n,:,(hh*stride):(hh*stride+HH),(ww*stride):(ww*stride+WW)] += w[f] * dout[n,f,hh,ww]
                  dw[f] += field * dout[n,f,hh,ww]
                  db[f] += dout[n,f,hh,ww]
...
def max_pool_forward_naive(x, pool_param):
  ...
  pool_h = 1 + (H - HH) / stride
  pool_w = 1 + (W - WW) / stride

  out = np.zeros((N, C, pool_h, pool_w))
  # idx = np.zeros_like(out)
  idx = np.zeros((N, C, pool_h, pool_w, len(x.shape)), dtype=np.int)
  window = None

  for n in xrange(N):
      for c in xrange(C):
          for hh in xrange(pool_h):  # 0,1,2
...
def max_pool_backward_naive(dout, cache):
  ...
  dx = np.zeros_like(x)
  for i, o in np.ndenumerate(dout):
      dx[tuple(idx[i])] = o
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
